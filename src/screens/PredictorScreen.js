...
const PredictorScreen = ({ navigation }) => {
    ...
    React.useEffect(() => {
        if (user?.role === 'student' && !user?.preferences?.isProfileComplete) {
            Alert.alert(
                'Profile Incomplete',
                'Please complete your admission profile (percentile, rank, etc.) to use the College Predictor.',
                [
...
const styles = StyleSheet.create({
    ...
    formCard: { marginVertical: Spacing.lg, padding: 20, marginTop: 0 },
    ...
    listContent: { paddingVertical: Spacing.lg, paddingTop: 0 },
